...
class CreateAdminForm(FlaskForm):
    username = StringField(
        "Brukernavn", validators=[DataRequired(), Length(min=3, max=20)]
    )
...
class MenuItemForm(FlaskForm):
    ...
    sort_order = IntegerField(
        "Rekkefølge", validators=[NumberRange(min=0, max=100)], default=10
    )
...
class CateringPackageForm(FlaskForm):
    ...
    items = TextAreaField(
        "Inkluderte retter (en per linje)", validators=[DataRequired()]
    )
    min_persons = IntegerField(
        "Minimum antall personer", validators=[NumberRange(min=1)], default=10
    )
...
class RestaurantInfoForm(FlaskForm):
    ...
    opening_hours = TextAreaField(
        "Åpningstider", validators=[DataRequired(), Length(max=500)]
    )
